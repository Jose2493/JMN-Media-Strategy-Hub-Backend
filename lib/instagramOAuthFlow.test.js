...
test('identity lookup errors retain sanitized diagnostics without claiming a mismatch', async () => {
  for (const code of ['PROVIDER_HTTP_ERROR', 'INVALID_RESPONSE', 'TIMEOUT', 'IDENTITY_MISMATCH']) {
    const trace = [];
    await assert.rejects(completeInstagramOAuthCallback(callbackArgs({
      ...happyDeps(trace),
      getAccountIdentity: async ({ expectedUserId }) => {
        if (!expectedUserId) return { userId: USER_ID, username: 'jmnmedia', accountType: 'BUSINESS' };
        throw Object.assign(new Error('secret provider response'), { code, status: 400 });
      },
    })), error => {
      assert.equal(error.code, code === 'IDENTITY_MISMATCH' ? code : 'IDENTITY_LOOKUP_FAILED');
      assert.equal(error.providerCode, code);
      assert.equal(error.providerStatus, 400);
      assert.equal(error.message.includes('secret'), false);
      return true;
    });
    assert.equal(trace.some(([step]) => step === 'encrypt' || step === 'upsert'), false);
  }
});
...
test('happy path verifies the same professional identity on both tokens', async () => {
  const trace = [];
  ...
  assert.deepEqual(trace.map(([name]) => name), [
    'consume', 'short', 'identity', 'long', 'identity', 'encrypt', 'upsert',
  ]);
...
test('OAuth scoped ID may differ; persistence and encryption use verified professional ID', async () => {
  const trace = [];
  const deps = happyDeps(trace);
  const result = await completeInstagramOAuthCallback(callbackArgs({
    ...deps,
    exchangeAuthorizationCode: async () => ({
      accessToken: 'IGAA_short_token_value', userId: '10200000000000001', permissions: REQUIRED_SCOPES,
    }),
  }));
  assert.equal(result.outcome, 'connected');
  const lookups = trace.filter(([name]) => name === 'identity');
  assert.equal(lookups[0][1].accessToken, 'IGAA_short_token_value');
  assert.equal(lookups[0][1].expectedUserId, undefined);
  assert.equal(lookups[1][1].accessToken, 'IGAA_long_token_value');
  assert.equal(lookups[1][1].expectedUserId, USER_ID);
  assert.equal(trace.find(([name]) => name === 'encrypt')[2].externalAccountId, USER_ID);
  assert.equal(trace.find(([name]) => name === 'upsert')[1].externalAccountId, USER_ID);
});

test('failed authorized identity lookup stops before token extension and persistence', async () => {
  const trace = [];
  await assert.rejects(completeInstagramOAuthCallback(callbackArgs({
    ...happyDeps(trace),
    getAccountIdentity: async () => { throw Object.assign(new Error('private'), {code:'TIMEOUT'}); },
  })), error => error.code === 'AUTHORIZED_IDENTITY_LOOKUP_FAILED' && error.providerCode === 'TIMEOUT');
  assert.deepEqual(trace.map(([name]) => name), ['consume', 'short']);
});
...
test('identity mismatch fails before encryption and persistence', async () => {
  ...
  await assert.rejects(
    completeInstagramOAuthCallback(callbackArgs({
      getAccountIdentity: async ({ expectedUserId }) => ({
        userId: expectedUserId ? '17841400000000002' : USER_ID,
        username: 'wrong',
...
test('provider and token errors are collapsed to static flow errors', async () => {
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.code, 'AUTHORIZATION_CODE_EXCHANGE_FAILED');
