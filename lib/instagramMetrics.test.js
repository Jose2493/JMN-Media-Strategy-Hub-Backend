...
import { getInstagramProfileCounts, getInstagramDailyReach, getInstagramRecentMedia } from './instagramClient.js';
...
function setup(overrides = {}) {
  ...
  const handler = createMetricsHandler({
    ...
    getProfile: async () => ({ followers: 12, following: 0 }), getReach: async () => [], getMedia: async () => [], ...overrides,
  });
...
test('recent media projects safe fields and uses video posters with missing counts intact', async () => {
  let seen;
  const posts = await getInstagramRecentMedia({ accessToken: 'private-token', userId, fetchImpl: async url => {
    seen = url; return json({ data: [
      {id:'123',media_type:'VIDEO',media_url:'https://evil.example/video',thumbnail_url:'https://scontent.cdninstagram.com/post.jpg',permalink:'https://www.instagram.com/reel/abc/',like_count:0,comments_count:null,access_token:'private-token'},
      {id:'124',media_type:'IMAGE',media_url:'https://cdninstagram.com.evil.example/x',permalink:'javascript:alert(1)',like_count:-1,timestamp:'bad'},
      {id:'125',media_type:'VIDEO',media_url:'https://scontent.cdninstagram.com/video.mp4'},
    ] });
  }});
  assert.equal(seen.pathname, `/${userId}/media`);
  assert.equal(seen.searchParams.get('limit'), '12');
  assert.equal(posts[0].thumbnailUrl, 'https://scontent.cdninstagram.com/post.jpg');
  assert.equal(posts[0].likes, 0); assert.equal(posts[0].comments, null);
  assert.equal(posts[1].thumbnailUrl, null); assert.equal(posts[1].permalink, null);
  assert.equal(posts[1].likes, null); assert.equal(posts[1].timestamp, null);
  assert.equal(posts[2].thumbnailUrl, null);
  assert.equal(JSON.stringify(posts).includes('private-token'), false);
});

test('media failure is partial and does not interrupt existing reach/profile', async () => {
  const app = setup({getMedia: async () => { throw Error('secret'); }});
  const res = await app.invoke();
  assert.equal(res.code, 200); assert.equal(res.body.recentMedia, null);
  assert.equal(res.body.profile.followers, 12); assert.deepEqual(res.body.dailyReach, []);
  assert.equal(res.body.partial, true); assert.equal(JSON.stringify(res.body).includes('secret'), false);
});
