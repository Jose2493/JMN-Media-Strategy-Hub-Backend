...
test('raw numeric Instagram IDs retain every digit across both exchanges', async () => {
  const result = await exchangeInstagramAuthorizationCode(validAuthArgs(async () => textResponse(
    '{"access_token":"IGAA_short_token_value","user_id":17841400000000001,"permissions":"instagram_business_basic"}'
  )));
  assert.equal(result.userId, VALID_USER_ID);
  const identity = await getInstagramAccountIdentity({
    accessToken: LONG_TOKEN,
    expectedUserId: result.userId,
    fetchImpl: async () => textResponse(
      '{"user_id":17841400000000001,"username":"jmnmedia","account_type":"BUSINESS"}'
    ),
  });
  assert.equal(identity.userId, VALID_USER_ID);
});
...
test('authorization exchange rejects fractional provider user_id', async () => {
  await assert.rejects(
    exchangeInstagramAuthorizationCode(validAuthArgs(async () => jsonResponse({
      data: [{ access_token: SHORT_TOKEN, user_id: 1.5, permissions: [] }],
    }))),
...
test('/me rejects fractional user_id', async () => {
  await assert.rejects(
    getInstagramAccountIdentity({
      accessToken: LONG_TOKEN,
      fetchImpl: async () => jsonResponse({ user_id: 1.5, username: 'jmnmedia' }),
    }),
    (error) => error instanceof InstagramClientError && error.code === 'INVALID_RESPONSE'
  );
});
