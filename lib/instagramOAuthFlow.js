...
export class InstagramOAuthFlowError extends Error {
  constructor(code = 'FLOW_FAILED', { providerCode = null, providerStatus = null } = {}) {
    super('Instagram connection failed');
    this.name = 'InstagramOAuthFlowError';
    this.code = code;
    this.providerCode = typeof providerCode === 'string' && /^[A-Z_]{1,64}$/.test(providerCode)
      ? providerCode
      : null;
    this.providerStatus = Number.isInteger(providerStatus) && providerStatus >= 100 && providerStatus <= 599
      ? providerStatus
      : null;
  }
}
...
function assertProfessionalIdentity(identity, expectedUserId) {
  ...
  if (typeof identity.userId !== 'string' || !APP_ID_PATTERN.test(identity.userId)) fail('INVALID_PROVIDER_RESPONSE');
...
export async function completeInstagramOAuthCallback({
  ...
  try {
    ...
    throw new InstagramOAuthFlowError('AUTHORIZATION_CODE_EXCHANGE_FAILED', {
  ...
  // Resolve the professional account ID from the authorized token itself.
  // OAuth's user_id and /me's user_id can belong to different ID namespaces.
  let authorizedIdentity;
  try {
    authorizedIdentity = await getAccountIdentity({ accessToken: shortLived.accessToken });
  } catch (error) {
    throw new InstagramOAuthFlowError('AUTHORIZED_IDENTITY_LOOKUP_FAILED', {
      providerCode: error?.code, providerStatus: error?.status,
    });
  }
  assertProfessionalIdentity(authorizedIdentity, authorizedIdentity?.userId);
  ...
  try {
    ...
    throw new InstagramOAuthFlowError('LONG_LIVED_TOKEN_EXCHANGE_FAILED', {
      providerCode: error?.code,
      providerStatus: error?.status,
    });
  }
  ...
  try {
    identity = await getAccountIdentity({
      ...
      expectedUserId: authorizedIdentity.userId,
    });
  } catch (error) {
    throw new InstagramOAuthFlowError(
      error?.code === 'IDENTITY_MISMATCH' ? 'IDENTITY_MISMATCH' : 'IDENTITY_LOOKUP_FAILED',
      { providerCode: error?.code, providerStatus: error?.status }
    );
  }
  assertProfessionalIdentity(identity, authorizedIdentity.userId);
