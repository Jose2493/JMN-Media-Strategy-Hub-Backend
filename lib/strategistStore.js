...
export function createStrategistStore(getDb = strategistDb) {
  const checked = async query => { const { data, error } = await query; if (error) throw httpError(503, 'Campaign workspace is temporarily unavailable'); return data; };
  return {
    ...
    async list(companyId) {
      const [campaigns, conversations] = await Promise.all([
        checked(getDb().from('strategist_campaigns').select('id,name,objective,created_at').eq('company_id',companyId).order('created_at',{ascending:false}).limit(100)),
        checked(getDb().from('conversations').select('id,created_at,last_message_at').eq('company_id',companyId).eq('channel','portal_ai_strategist').order('created_at',{ascending:false}).limit(200)),
      ]);
