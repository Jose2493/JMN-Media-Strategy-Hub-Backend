...
// Server-only credential lookup. Account IDs supplied by the browser never
// establish ownership: the verified session company must match the row.
export async function getInstagramAccountForMetrics(companyId, accountId) {
  assertUuid(companyId);
  assertUuid(accountId);
  const { data, error } = await getSupabaseClient()
    .from('social_accounts')
    .select('id, company_id, external_account_id, status, token_expires_at, scopes, updated_at, access_token_ciphertext, access_token_iv, access_token_auth_tag, token_key_version, token_aad_version')
    .eq('company_id', companyId)
    .eq('id', accountId)
    .eq('platform', 'instagram')
    .maybeSingle();
  if (error) throw new SocialAccountsError();
  return data;
}
