...
const MEMORY_UPDATE_THRESHOLD = 2;
...
Only include fields that genuinely changed or are newly learned. If nothing new, return empty objects/arrays.`;
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({ model: 'claude-opus-4-6', max_tokens: 500, messages: [{ role: 'user', content: extractionPrompt }] })
  });
  ...
  let extracted;
  try {
    extracted = JSON.parse(data.content?.[0]?.text?.trim() || '{}');
  } catch (e) {
