...
export async function deleteInstagramAccountDataByExternalId(externalAccountId, {
  ...
  // Remove private publisher uploads before deleting the account cascade.
  // Iterate by job ID (without mutating rows) so every page is visited.
  let after = null;
  while (true) {
    let query = client.from('social_publish_jobs').select('id,storage_path').eq('account_id', accountId).order('id').limit(100);
    if (after) query = query.gt('id', after);
    const {data: uploads, error: uploadError} = await query;
    // Additive rollout may precede migration; other database failures must fail closed.
    if (uploadError && ['42P01','PGRST205'].includes(uploadError.code)) break;
    if (uploadError || !Array.isArray(uploads)) fail();
    if (!uploads.length) break;
    const {error: storageError} = await client.storage.from('social-publishing').remove(uploads.map(row => row.storage_path));
    if (storageError) fail();
    after = uploads[uploads.length - 1].id;
  }
