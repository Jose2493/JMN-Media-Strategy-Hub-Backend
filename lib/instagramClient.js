...
async function requestJson({ url, options, fetchImpl, timeoutMs, operation }) {
  ...
  try {
    ...
    let payload;
    try {
      payload = JSON.parse(text, (key, value, context) => {
        if (key !== 'user_id' || typeof value !== 'number') return value;
        // Read the original JSON integer, never an already-rounded JS number.
        // Runtimes without source support accept only provably safe integers.
        const source = context?.source;
        if (typeof source === 'string' && USER_ID_PATTERN.test(source)) return source;
        if (source === undefined && Number.isSafeInteger(value) && value >= 0) return String(value);
        return value; // Existing strict validation rejects all other forms.
      });
    } catch {
      throw new InstagramClientError('INVALID_RESPONSE', { operation });
    }
...
// Deliberately limited to profile counts and daily reach. Missing provider
// values stay null/empty; daily unique reach must never be summed as weekly reach.
export async function getInstagramProfileCounts({ accessToken, expectedUserId, fetchImpl = globalThis.fetch } = {}) {
  const operation = 'get_profile_counts';
  requireString(accessToken, operation, { maxLength: MAX_TOKEN_LENGTH });
  assertInstagramUserId(expectedUserId);
  const url = new URL('/me', INSTAGRAM_GRAPH_BASE_URL);
  url.searchParams.set('fields', 'user_id,followers_count,follows_count');
  url.searchParams.set('access_token', accessToken);
  const payload = await requestJson({ url, options: { method: 'GET' }, fetchImpl, timeoutMs: DEFAULT_INSTAGRAM_TIMEOUT_MS, operation });
  if (assertInstagramUserIdFromProvider(payload.user_id, operation) !== expectedUserId) {
    throw new InstagramClientError('IDENTITY_MISMATCH', { operation });
  }
  const count = value => Number.isSafeInteger(value) && value >= 0 ? value : null;
  return { followers: count(payload.followers_count), following: count(payload.follows_count) };
}

export async function getInstagramDailyReach({ accessToken, userId, since, until, fetchImpl = globalThis.fetch } = {}) {
  const operation = 'get_daily_reach';
  requireString(accessToken, operation, { maxLength: MAX_TOKEN_LENGTH });
  assertInstagramUserId(userId);
  if (!Number.isSafeInteger(since) || !Number.isSafeInteger(until) || since < 1 || until <= since || until - since > 7 * 86400) throw invalidInput(operation);
  const url = new URL(`/${userId}/insights`, INSTAGRAM_GRAPH_BASE_URL);
  for (const [key, value] of Object.entries({ metric: 'reach', period: 'day', metric_type: 'time_series', since, until, access_token: accessToken })) url.searchParams.set(key, String(value));
  const payload = await requestJson({ url, options: { method: 'GET' }, fetchImpl, timeoutMs: DEFAULT_INSTAGRAM_TIMEOUT_MS, operation });
  if (!Array.isArray(payload.data)) throw new InstagramClientError('INVALID_RESPONSE', { operation });
  const metric = payload.data.find(item => item?.name === 'reach' && item.period === 'day');
  if (!metric) return [];
  if (!Array.isArray(metric.values) || metric.values.length > 32) throw new InstagramClientError('INVALID_RESPONSE', { operation });
  return metric.values.map(point => {
    const end = Date.parse(point?.end_time);
    if (!Number.isFinite(end)) throw new InstagramClientError('INVALID_RESPONSE', { operation });
    return { endTime: new Date(end).toISOString(), value: Number.isSafeInteger(point.value) && point.value >= 0 ? point.value : null };
  }).filter(point => Date.parse(point.endTime) > since * 1000 && Date.parse(point.endTime) <= until * 1000)
    .sort((a, b) => a.endTime.localeCompare(b.endTime));
}

export async function getInstagramRecentMedia({ accessToken, userId, fetchImpl = globalThis.fetch } = {}) {
  const operation = 'get_recent_media';
  requireString(accessToken, operation, { maxLength: MAX_TOKEN_LENGTH });
  assertInstagramUserId(userId);
  const url = new URL(`/${userId}/media`, INSTAGRAM_GRAPH_BASE_URL);
  ...
  const safeUrl = (value, image) => {
    try {
      ...
      const allowed = image ? ['cdninstagram.com', 'fbcdn.net'].some(host => u.hostname === host || u.hostname.endsWith('.' + host)) : ['instagram.com', 'www.instagram.com'].includes(u.hostname);
      return u.protocol === 'https:' && !u.username && !u.password && !u.port && allowed ? u.href : null;
    } catch { return null; }
  };
  const count = value => Number.isSafeInteger(value) && value >= 0 ? value : null;
  return payload.data.slice(0, 12).filter(item => item && typeof item.id === 'string' && /^\d+$/.test(item.id)).map(item => ({
    id: item.id, caption: typeof item.caption === 'string' ? item.caption.slice(0, 2200) : '',
    type: ['IMAGE','VIDEO','CAROUSEL_ALBUM'].includes(item.media_type) ? item.media_type : 'UNKNOWN',
    thumbnailUrl: safeUrl(item.media_type === 'VIDEO' ? item.thumbnail_url : item.media_url, true),
    permalink: safeUrl(item.permalink, false),
    timestamp: Number.isFinite(Date.parse(item.timestamp)) ? new Date(item.timestamp).toISOString() : null,
    likes: count(item.like_count), comments: count(item.comments_count),
  }));
}
