export function formatCampaignContext(metadata) {
  if (!metadata) return '';
  const action=metadata.social_context?.kind==='social_action';
  const context = {campaign:metadata.campaign ? {name:metadata.campaign.name,objective:metadata.campaign.objective} : null,conversationTitle:metadata.title,instagramSnapshot:action?null:metadata.social_context || null,actionBrief:action?metadata.social_context:null};
...
An action brief is a goal-based experiment, not a finding from Instagram analytics. Confirm missing offer, audience, production constraints, and measurement details. Incorporate the client's adjustment reason when adapting the draft. Manually reported inquiries/bookings are self-reported, not verified attribution. A request for JMN is a draft the client must share; never claim it was sent to a human team.
